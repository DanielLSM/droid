...
namespace UnityStandardAssets.CrossPlatformInput {
  public abstract class VirtualInput {
    ...
    public bool AxisExists(string name) { return this.m_VirtualAxes.ContainsKey(name); }

    public bool ButtonExists(string name) { return this.m_VirtualButtons.ContainsKey(name); }
    ...
    public void RegisterVirtualAxis(CrossPlatformInputManager.VirtualAxis axis) {
      ...
      if (this.m_VirtualAxes.ContainsKey(axis.name))
        ...
        if (!axis.matchWithInputManager) this.m_AlwaysUseVirtual.Add(axis.name);
      }
    }
    ...
    public void RegisterVirtualButton(CrossPlatformInputManager.VirtualButton button) {
      ...
      if (this.m_VirtualButtons.ContainsKey(button.name))
        ...
        if (!button.matchWithInputManager) this.m_AlwaysUseVirtual.Add(button.name);
      }
    }

    public void UnRegisterVirtualAxis(string name) {
      // if we have an axis with that name then remove it from our dictionary of registered axes
      if (this.m_VirtualAxes.ContainsKey(name)) this.m_VirtualAxes.Remove(name);
    }

    public void UnRegisterVirtualButton(string name) {
      // if we have a button with this name then remove it from our dictionary of registered buttons
      if (this.m_VirtualButtons.ContainsKey(name)) this.m_VirtualButtons.Remove(name);
    }
