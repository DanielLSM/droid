...
namespace SceneAssets.ImagePuzzler {
  public class Puzzle : MonoBehaviour {
    ...
    [SerializeField]
    int _shuffle_length = 20;
    [SerializeField]
    float _default_move_duration = .2f;
    [SerializeField]
    float _shuffle_move_duration = .1f;
    ...
    void Update () {
      if (this._state == PuzzleState.Solved && Input.GetKeyDown (KeyCode.Space)) {
        this.StartShuffle ();
      }
    }
    ...
    void PlayerMoveBlockInput (Block block_to_move) {
      if (this._state == PuzzleState.InPlay) {
    ...
    void OnBlockFinishedMoving () {
      ...
      if (this._state == PuzzleState.InPlay) {
        this.MakeNextPlayerMove ();
      } else if (this._state == PuzzleState.Shuffling) {
        if (this._shuffle_moves_remaining > 0) {
          this.MakeNextShuffleMove ();
        } else {
          this._state = PuzzleState.InPlay;
        }
      }
    }

    void StartShuffle () {
      this._state = PuzzleState.Shuffling;
    ...
    void MakeNextShuffleMove () {
      ...
      for (var i = 0; i < offsets.Length; i++) {
        ...
        if (offset != this._prev_shuffle_offset * -1) {
          ...
          if (move_block_coord.x >= 0 && move_block_coord.x < this._horisontal_divisions && move_block_coord.y >= 0 && move_block_coord.y < this._vertical_divisions) {
    ...
    void CheckIfSolved () {
      ...
      this._state = PuzzleState.Solved;
